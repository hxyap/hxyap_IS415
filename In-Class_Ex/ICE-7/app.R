...
hunan_profile <- left_join(hunan, hunan2012, by = c("County" = "County"))
#========================#
###### Shiny UI ######
#========================#  
...
ui <- navbarPage(
  ...
  fluid = TRUE, # same as use fluid page
  ...
  navbarMenu("Global Measures", # code for our navbar
             tabPanel("Moran's I"),
             tabPanel("Geary's c"),
             tabPanel("Getis-Ord Global G")
             ),
  navbarMenu("Local Measures",
             tabPanel("Local Moran", # code for our tab
                      sidebarLayout(
                        sidebarPanel(
                          selectInput(inputId = "variable",
                                      label = "Mapping variable",
                          ...
                          radioButtons(inputId = "Contiguity1",
                                       ...
                                       choices = c("Queen" = TRUE, 
                                                   "Rook" = FALSE), # gives you toggle buttons
                                       selected = "TRUE",
                                       inline = TRUE),
...
server <- function(input, output){
    output$mapPlot <- renderTmap({
      tmap_options(check.and.fix = TRUE) +
        ...
        tm_view(set.zoom.limits = c(6.5, 8) # controls zoom limits 
                ) # lowers resource requirements on webapp
    })
    ...
    localMIResults <- eventReactive(input$MoranUpdate,{
      ...
      wm_q <- hunan_profile %>%
        mutate(nb = st_contiguity(geometry, 
                                  queen = !!input$Contiguity1), # this part too
               wt = st_weights(nb,
                               style = input$MoranWeights)) # ensures no vector issue
